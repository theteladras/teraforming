...
resource "aws_elb" "loadbalancer_teraforming" {
  name            = "teraforming"
  instances       = aws_instance.instance_teraforming_prod.*.id
  subnets         = [aws_default_subnet.subnet_az1.id, aws_default_subnet.subnet_az2.id]
  security_groups = [aws_security_group.sec_teraforming_prod.id]
